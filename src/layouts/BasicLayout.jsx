...
const formatMenu = (menuData = [], auth = [], parentName) => {
  return menuData
    .map((item) => {
      ...
      if (item.routes) result.children = formatMenu(item.routes, auth, locale);
...
class BasicLayout extends PureComponent {
  ...
  componentDidUpdate(preProps, preState, { pathname }) {
    if (preProps.location.pathname !== pathname)
      this.breadMap = this.getBreadMap();
  }
  render() {
    const { menuData } = this.state;
    return (
      <Block w="100%" h="100%" vf>
        <Header />
        <Block f={1} wf>
